...
// GPU heat: blue (idle) → violet → magenta (hot). Deliberately a different ramp
// than CPU's green→red so the two columns don't read as the same signal.
func gpuColor(_ gpu: Double) -> Color {
    let g = min(max(gpu, 0), 100) / 100
    return Color(hue: 0.60 + 0.25 * g, saturation: 0.72, brightness: 0.95)
}
    ...
    return snap.nodes.map { n in
        let kids: [RowItem]? = n.children.count > 1 ? n.children.map { c in
            RowItem(id: "\(n.id)#\(c.pid)", label: "\(c.label)  #\(c.pid)", kind: "child",
                    cpu: c.cpu, gpu: c.gpu, rss: c.rss_mb, procs: 1, ports: [], conns: 0, cmd: c.cmd, children: nil)
        } : nil
        return RowItem(id: n.id, label: n.label, kind: n.kind, cpu: n.cpu, gpu: n.gpu, rss: n.rss_mb,
